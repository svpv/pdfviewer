...
int PBTouchZoomDlg::MainHandler(int type, int par1, int par2)
{
	...
	{
		if (par1 == KEY_UP || par1 == KEY_DOWN || par1 == KEY_LEFT || par1 == KEY_RIGHT)
		{
...
int Increment(std::vector<int>& v, int current)
{
...
int Decrement(std::vector<int>& v, int current)
{
...
void PBTouchZoomDlg::GetNextZoomParam(TZoomType zoom_type, int zoom_param, bool decrement, TZoomType & new_zoom_type, int & new_zoom_param)
{
	const int SC_PREVIEW[] = { 33, 50 };
	const int SC_NORMAL[] =	{ 75, 80, 85, 90, 95, 100, 105, 110, 120, 130, 140, 150, 170 };
	const int SC_COLUMNS[] = { 200, 300, 400, 500 };
	...
	new_zoom_param = zoom_param;

	if (zoom_type == ZoomTypeFitWidth)
	{
		if (decrement)
		{
			new_zoom_type = ZoomTypeNormal;
			new_zoom_param = 95;
		}
		else
		{
	...
	else if (zoom_type == ZoomTypeNormal)
	{
		std::vector<int> v(SC_NORMAL, SC_NORMAL + sizeof(SC_NORMAL)/sizeof(*SC_NORMAL));
		new_zoom_param = decrement ? Decrement(v, zoom_param) : Increment(v, zoom_param);
		if (new_zoom_param == -1)
		{
			if (decrement)
			{
				new_zoom_type = ZoomTypePreview;
				new_zoom_param = 50;
			}
			else
			{
				new_zoom_type = ZoomTypeColumns;
				new_zoom_param = 200;
			}
		}
	}
	...
	{
		std::vector<int> v(SC_REFLOW, SC_REFLOW + sizeof(SC_REFLOW)/sizeof(*SC_REFLOW) );
		new_zoom_param = decrement ? DecrementCyclically(v, zoom_param) : IncrementCyclically(v, zoom_param);
	}
	else if (zoom_type == ZoomTypeEpub)
	{
		std::vector<int> v(SC_EPUB, SC_EPUB + sizeof(SC_EPUB)/sizeof(*SC_EPUB) );
